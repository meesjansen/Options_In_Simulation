...
class TorqueDistributionTask(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
       ...
       # --- KA-DDPG/KA-PPO modifications: use a state space of 4 and action space of 4 ---
        self._num_observations = 6  # [v_desired, omega_desired, v_delta, omega_delta, linear_acc, angular_acc]
        self._num_actions = 1       # [T_fl,_rl,_fr,_rr]
        ...
        self.max_global_episodes = 50.0
        self.max_sim_steps = 50000.0 # 250 episodes of 10s at 100Hz sim and 10Hz control/policy step
        # ---------------------------------------------------------------------------
        ...
        self.gamma_assist = torch.zeros(self.num_envs, dtype=torch.float)
    ...
    def update_config(self, sim_config):
        ...
        self.evalsampling = self._task_cfg["env"]["terrain"]["EvalSampling"]
    ...
    def sample_velocity_command(self, env_id: int):
        ...
        if self.evalsampling:
            # Linearly increase x_vel from 1.0 to 2.0 over the course of training
            progress = torch.clamp(self.sim_steps.float()[env_id] / self.max_sim_steps, 0.0, 1.0)
            x_vel = 1.0 + progress * (2.0 - 1.0)
            omega = 0.0
