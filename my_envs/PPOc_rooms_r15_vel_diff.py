...
class ReachingTargetTask(RLTask):
    ...
    def pre_physics_step(self, actions):
        ...
        self.min_vel = -1.0 * self.action_scale
        self.max_vel = 1.0 * self.action_scale
        ...
        base_vel = self.min_vel + (self.actions[:, 0] + 1) * 0.5 * (self.max_vel- self.min_vel)
        # Compute the delta/differential torque:
        delta_vel = self.min_delta + (self.actions[:, 1] + 1) * 0.5 * (self.max_delta - self.min_delta)
        ...
        for _ in range(self.decimation):
            if self.world.is_playing():
                ...
                self._robots.set_joint_velocities(updated_rad)
