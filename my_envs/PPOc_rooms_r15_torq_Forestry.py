    ...
    def pre_physics_step(self, actions):
        ...
        for _ in range(self.decimation):
            if self.world.is_playing():
                ...
                desired_wheel_speed = self.action_scale * self.actions  # shape: [num_wheels]
                self.torq_error = desired_wheel_speed - (self.r * self.dof_vel)
                self.torq_error_der = (self.torq_error - self.last_torq_error) / self.sim_dt
                FF_wheel_torq = self.torq_FF_gain * desired_wheel_speed + self.torq_constant
                wheel_torq_corrections = self.Kp * self.torq_error + self.Kd * self.torq_error_der
                raw_wheel_torqs = FF_wheel_torq + wheel_torq_corrections
                ...
                raw_wheel_torqs[clamp_mask] = 0.0

                wheel_torqs = torch.clip(raw_wheel_torqs, -80.0, 80.0)
