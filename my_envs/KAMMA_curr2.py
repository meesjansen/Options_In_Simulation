...
class TorqueDistributionTask(RLTask):
    def __init__(self, name, sim_config, env, offset=None) -> None:
       ...
       # --- KA-DDPG/KA-PPO modifications: use a state space of 4 and action space of 4 ---
        self._num_observations = 6  # [v_delta, omega_delta, linear_acc, angular_acc, v_forward, v_lateral]
        ...
        self.max_sim_steps = 500000.0 # 250 episodes of 10s at 100Hz sim and 10Hz control/policy step
        # ---------------------------------------------------------------------------
    ...
    def sample_velocity_command(self, env_id: int):
        ...
        if self.curriculum:
            ...
            x_left_mean = torch.tensor(x_left_mean, device=self.device).unsqueeze(0)
            x_right_mean = x_center + progress * (x_high - x_center)
            x_right_mean = torch.tensor(x_right_mean, device=self.device).unsqueeze(0)
            ...
            if torch.rand(1, device=self.device).item() < 0.5:
                x_vel = torch.normal(mean=x_left_mean, std=fixed_std).item()
            else:
                x_vel = torch.normal(mean=x_right_mean, std=fixed_std).item()
            ...
            yaw_left_mean = torch.tensor(yaw_left_mean, device=self.device).unsqueeze(0)
            yaw_right_mean = yaw_center + progress * (yaw_high - yaw_center)
            yaw_right_mean = torch.tensor(yaw_right_mean, device=self.device).unsqueeze(0)

            if torch.rand(1, device=self.device).item() < 0.5:
                omega = torch.normal(mean=yaw_left_mean, std=fixed_std).item()
            else:
                omega = torch.normal(mean=yaw_right_mean, std=fixed_std).item()
