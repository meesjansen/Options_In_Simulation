...
class ReachingTargetTask(RLTask):
    ...
    def calculate_metrics(self) -> None:
        ...
        self.lambda_slip = (v_wheel - self.base_lin_vel[:, 0]) / np.maximum(v_wheel, self.base_lin_vel[:, 0])  # avoid divide by zero
        self.k_lambda = 0.3

        # Longitudinal slip reward
        rew_slip_longitudinal = np.prod(np.exp(-0.5 * (self.lambda_slip / self.k_lambda) ** 2)) * self.rew_scales["slip_longitudinal"]     
