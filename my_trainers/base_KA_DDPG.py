    ...
    def single_agent_train(self) -> None:
        ...
        for timestep in tqdm.tqdm(range(self.initial_timestep, self.timesteps), disable=self.disable_progressbar, file=sys.stdout):
            ...
            with torch.no_grad():
                ...
                torque_fl = self.env.task.observed_components["env0_policy_torque_fl"]
                torque_rl = self.env.task.observed_components["env0_policy_torque_rl"]
                torque_fr = self.env.task.observed_components["env0_policy_torque_fr"]
                torque_rr = self.env.task.observed_components["env0_policy_torque_rr"]
