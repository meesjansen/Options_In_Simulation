...
def compute_normal_map(height_field_raw, horizontal_scale, vertical_scale):
    """
    Compute normal vectors (Nx, Ny, Nz) for each point in the height field.
    ...
    Parameters:
        height_field_raw (np.array): The heightfield array of shape (W, L).
        horizontal_scale (float): Horizontal scaling factor (m/px).
        vertical_scale (float): Vertical scaling factor (m/height_unit).

    Returns:
        normal_map_x, normal_map_y, normal_map_z (np.array): Normal maps for each axis.
    """
    # Convert height_field_raw to float for gradient computation
    hf = height_field_raw.astype(np.float32) * vertical_scale
    W, L = hf.shape

    # Compute partial derivatives using finite differences
    # For boundaries, we replicate the nearest value
    dzdx = np.zeros_like(hf)
    dzdy = np.zeros_like(hf)

    dzdx[:-1, :] = hf[1:, :] - hf[:-1, :]
    dzdy[:, :-1] = hf[:, 1:] - hf[:, :-1]

    # Scale derivatives by horizontal_scale
    # dz/dx and dz/dy in world units: divide by horizontal_scale
    dzdx /= horizontal_scale
    dzdy /= horizontal_scale

    # Compute unnormalized normals: N = (-dz/dx, -dz/dy, 1)
    Nx = -dzdx
    Ny = -dzdy
    Nz = np.ones_like(hf)
