    ...
    def _update(self, timestep: int, timesteps: int) -> None:
        def compute_gae(rewards: torch.Tensor,
            ...
            for i in reversed(range(memory_size)):
                next_values_val = values[i + 1] if i < memory_size - 1 else last_values
                advantage = rewards[i] - values[i] + discount_factor * not_dones[i] * (next_values_val + lambda_coefficient * advantage)
