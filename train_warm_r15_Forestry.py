...
if WARM_START:
    ...
    # Enable warm-start mode in the environment so that it applies the heuristic actions.
    task.warm_start = True
    # Freeze the value network (disable its updates).
    ...
    for step in range(WARM_START_TIMESTEPS):
         ...
         # Unpack 5 values from env.step (obs, reward, terminated, truncated, extras)
    ...
    # Disable warm-start mode in the environment.
